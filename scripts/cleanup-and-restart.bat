@echo off
REM Script dọn dẹp container cũ và khởi động lại với tên mới
REM Sử dụng: cleanup-and-restart.bat

echo 🧹 Dọn dẹp containers cũ...

REM Dừng và xóa containers cũ
echo ⏹️ Dừng containers cũ...
docker stop ordercheck-app ordercheck-mongodb 2>nul

echo 🗑️ Xóa containers cũ...
docker rm ordercheck-app ordercheck-mongodb 2>nul

REM Dừng và xóa containers hiện tại (nếu có)
echo ⏹️ Dừng containers hiện tại...
docker stop ordercheck-app-v2 ordercheck-mongodb-v2 2>nul

echo 🗑️ Xóa containers hiện tại...
docker rm ordercheck-app-v2 ordercheck-mongodb-v2 2>nul
